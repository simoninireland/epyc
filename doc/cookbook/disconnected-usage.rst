...
**Problem**: You're using a remote machine to run your simulations on,
and don't want your local machine to have to stay connected while
they're running because you're doing a lot of computation.

**Solution**: ``epyc``'s cluster labs can work asynchronously, so you
...
:class:`ClusterLab`. Starting experiments by default creates a pending
...
(This is an important use case especially when using a remote cluster
with a Jupyter notebook, detailed more in the :ref:`fourth-tutorial`.)
The notebook will gradually be emptied of pending results and filled
with completed results, until none remain.
...
The system for retrieving completed results is quite robust in that it
commits the notebook as results come in, minimising the posibility for
loss through a crash.
...
The call to :meth:`ClusterLab.updateResults` connects to the cluster
